...
import kr.ac.uos.je.model.EMapManager.MapStatus;
...
public enum EMapFactory {
	...
	public void updateMap(EMapManager EMapManager, DataPacket packet ) {
		...
		if (_isNextPacketLines) {
			...
			int count = iter.nextByte4();
			if (count > 0) {
				parsePoint(EMapManager, iter, count);
			} else {
				// DATA 정보 수신 완료 
...
//	}
	...
	public void parseLine(EMapManager EMapManager, DataPacketIterator iter, int count) {
		for (int i=0; i<count; i++) {
			int x1 = iter.nextByte4();
			int y1 = iter.nextByte4();
			int x2 = iter.nextByte4();
			int y2 = iter.nextByte4();
			EMapManager.addMapLine(x1, y1, 0, x2, y2, 0);
		}
	}
	
	public void parsePoint(EMapManager EMapManager, DataPacketIterator iter, int count) {
		for (int i=0; i<count; i++) {
			int x = iter.nextByte4();
			int y = iter.nextByte4();
			EMapManager.addMapPoint(x, y, 0);
		}
	}

	
	public void parseMinPos(EMapManager EMapManager, String sentence) {
	...
	public void parseMaxPos(EMapManager EMapManager, String sentence) {
		String[] separetedSentence = sentence.split(" ");
		int x = Integer.valueOf(separetedSentence[1]);
		int y = Integer.valueOf(separetedSentence[2]);
		EMapManager.setMinPos(x,y);
	...
	public void parseResolution(EMapManager EMapManager, String sentence) {
		String[] separetedSentence = sentence.split(" ");
		int resolution = Integer.parseInt(separetedSentence[1]);
		EMapManager.setResolution(resolution);
	}

	
	public void parseMapObject(EMapManager EMapManager, String sentence) {
		MapObjectFactory.parseObjectWithSentece(EMapManager, sentence);
	}
	...
	private static class MapObjectFactory{
		...
		public static void parseObjectWithSentece(EMapManager EMapManager, String sentence){
			...
			if (type.equals(kMapBinaryRobotHome)){
				EMapManager.setRobotHome(x, y, description, iconName,  name); 
			} else if (type.equals(kMapBinaryGoal)) {
				EMapManager.addGoal(x,y,  description, iconName,  name, false);  
			} else if (type.equals(kMapBinaryGoalWithHeading)) {
				EMapManager.addGoal(x,y,  description, iconName,  name, true);
				...
				EMapManager.addForbiddenLine(x1,y1,x2,y2,description, iconName ,name);
			} else if (type.equals(kMapBinaryForbiddenArea)) {
				Integer x1 = Integer.valueOf(aSentence[index++]);
				Integer y1 = Integer.valueOf(aSentence[index++]);
				Integer x2 = Integer.valueOf(aSentence[index++]);
				Integer y2 = Integer.valueOf(aSentence[index++]);
				EMapManager.addForbiddenArea(x1,y1,x2,y2,description, iconName ,name);
			} else {
